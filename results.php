...
<header>
							...
							<div class="main-menu text-right hidden-xs hidden-sm">
								<nav style="float:left;">
									<ul class="basic-menu">
										...
										<li><a href="results.php">Получить результаты</a></li>
									</ul>
								</nav>
							</div>
							<!-- basic-mobile-menu --> 
	...
	if($_SERVER['REQUEST_METHOD']=='POST' && isset($_POST['sign-in'])){
		...
		$query = "SELECT * FROM results WHERE code='$password'";
		$sql = $link->query($query);
		$n = $sql->num_rows;
		if($n > 0){
		    $_SESSION['code'] = $password;
			//header('Location: getresult.php');
			echo "<script>window.location.href='getresult.php';</script>";
		}else {
			echo'<script>
			if(confirm("Неверный код")===true){
		...
		<div class="basic-breadcrumb-area bg-opacity bg-1 ptb-100">
			<div class="container">
				<div class="basic-breadcrumb text-center">
					<h3 class="">Получение результатов</h3>
					<ol class="breadcrumb text-xs">
						<li><a href="index.html">Главная</a></li>
						<li class="active">Получение результатов</li>
				...
				<div class="row">
					<div class="col-sm-4 col-sm-offset-4">
						<div class="account-form">
							<h4 class="text-center mb-30">Получение результатов</h4>
							<form method="post" action="">

								<div class="form-group">
									
									<label class="sr-only">Кодовое слово</label>
									<input type="password" class="form-control input-lg" placeholder="Пароль" name="loginpw">
								</div>

								<div class="form-group">
									<button class="btn btn-block btn-round btn-base" name="sign-in">Получить результаты</button>
								</div>
							</form>						
						</div>
					</div>
