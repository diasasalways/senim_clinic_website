...
<header>
		...
		<div class="basic-breadcrumb-area bg-opacity bg-1 ptb-100">
			<div class="container">
				<div class="basic-breadcrumb text-center">
					<h3 class="">Изменить доктора</h3>
					<ol class="breadcrumb text-xs">
						<li><a href="index.html">Главная</a></li>
						<li class="active"><a href="#">Изменить доктора</a></li>
		...
		$id = $_GET['id'];
		$values = mysqli_query($link,("SELECT * FROM `doctors` WHERE `id`='$id' "));
				...
				<div class="row">
					<div class="col-sm-4 col-sm-offset-4">
						<div class="account-form">
							<h4 class="text-center mb-30">Изменить доктора</h4>
							<form method="post" action=""  enctype = 'multipart/form-data'>
								<div class="form-group">
									<label class="sr-only">ФИО</label>
									<input type="text" class="form-control input-lg" placeholder="ФИО" name="name" value="<?= $value['doctor_name']; ?>"  >
								</div>
								<div class="form-group">
									<label class="sr-only">Специальность</label>
									<input type="text" class="form-control input-lg" placeholder="Специальность" name="spec"  value="<?= $value['speciality']; ?>">
								</div>
								<div class="form-group">
									<label class="sr-only">Стаж работы</label>
									<input type="text" class="form-control input-lg" placeholder="Стаж работы" name="sr"  value="<?= $value['description']; ?>">
								</div>
								<div class="form-group">
									<label class="sr-only">Категория</label>
									<input type="text" class="form-control input-lg" placeholder="Категория" name="category"  value="<?= $value['study']; ?>" >
								</div>
								<div class="form-group">
									<label class="sr-only">График работы</label>
									<input type="text" class="form-control input-lg" placeholder="График работы" name="worktime"  value="<?= $value['time']; ?>">
								</div>
								<div class="form-group">
...
$uploadpath='doctors/'.$uploadname; //path of content
		...
		$upd = "UPDATE `doctors` SET `doctor_name`='$name', `description`='$sr', `speciality`='$spec', `study`='$category', `time`='$worktime' 
				WHERE `id`='$id' "; //update password of user
